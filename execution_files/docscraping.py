...
def regulatory_compliance(item1a, ticker):
    ...
    if ticker in pre: # match the paragrah that starts with "Risks Related to"

        pattern = r'\n\n(Risks Relat\w+? to [^\n]+)\s+'
  
    elif ticker in post: # match the paragrah that ends with "Risks"

        text = text_preprocessing(text)

        pattern = r'[A-Z][A-Za-z,\s]* Risks'

    if ticker in irregular: # irregular writing styles of some 10-K reports
